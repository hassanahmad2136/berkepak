  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto backdrop-blur-xl bg-paper/95 px-4 sm:px-8 py-10 transition-all duration-300 ease-out fade-in">
      <div className="mx-auto max-w-[900px] w-full flex flex-col h-full min-h-[80vh]">
        ...
        <div className="mt-10 flex-1 grid gap-8 md:grid-cols-[220px_1fr]">
          {/* Left panel: Suggested categories */}
          <aside className="border-r border-stone/20 pr-4 md:block hidden min-h-[300px]">
            <p className="eyebrow text-muted text-[11px] mb-4">Categories</p>
            {matchedCategories.length > 0 ? (
                ...
                {matchedCategories.map((c) => (
                  <li key={c.slug}>
                    <Link
                      ...
                      onClick={onClose}
                      className="inline-block px-3 py-1 text-xs uppercase tracking-[0.14em] border border-stone/60 hover:border-ink rounded-full transition-colors text-ink bg-white"
                    >
                      {c.label}
                    </Link>
                  </li>
              ...
              <p className="text-xs text-muted">No category matches.</p>
            ) : (
                ...
                {CATEGORIES.map((c) => (
                  <li key={c.slug}>
                    <Link
                      href={`/shop?category=${c.slug}`}
                      onClick={onClose}
                      className="link-underline text-xs uppercase tracking-[0.12em] text-ink-soft hover:text-ink"
                    >
                      {c.label}
                    </Link>
                  </li>
          ...
          {/* Right panel: Fabric details matches */}
            ...
            {!query ? (
              ...
              <div className="py-12 text-center sm:text-left">
                ...
                <p className="text-sm text-muted mt-2">
                  Try adjusting spelling or searching for materials like "cotton", "silk", or "wool".
                </p>
              </div>
            ) : (
